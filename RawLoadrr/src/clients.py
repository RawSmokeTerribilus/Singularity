...
from src.logger import get_logger # Added for structured logging
...
class Clients():
    ...
    def __init__(self, config):
        self.config = config
        self.logger = get_logger('CLIENTS') # Initialize logger here
        pass
    ...
    async def is_valid_torrent(self, meta, torrent_path, torrenthash, torrent_client, print_err=False):
        ...
        if torrent_client in ('qbit', 'deluge'):
            ...
            torrent_path = torrent_path.replace(torrenthash.upper(), torrenthash)
        if meta['debug']:
            self.logger.info(f"Torrent path: {torrent_path}")
        if os.path.exists(torrent_path):
            torrent = Torrent.read(torrent_path)
            ...
            # Check if number of files matches number of videos
                ...
                if meta['debug']:
                    self.logger.info(f"torrent_filepath: {torrent_filepath}")
                    self.logger.info(f"actual_filepath: {actual_filepath}")
    ...
    async def qbittorrent(self, path, torrent, local_path, remote_path, client, is_disc, filelist, meta, tracker):
        ...
        if meta['debug']:
            self.logger.info(f"meta['debug'] is {meta['debug']}") 
            self.logger.info(f"Adding torrent with infohash: {torrent.infohash}")
            self.logger.info(f"Save path: {path}")
        ...
        for i in range(30):
            if meta['debug']:
                self.logger.info(f"Attempt {i+1}: Checking for infohash {torrent.infohash} in qBittorrent...")
            torrents = qbt_client.torrents_info(torrent_hashes=torrent.infohash)
            if meta['debug']:
                self.logger.info(f"qbt_client.torrents_info result: {torrents}")
            if torrents:
                torrent_found = True
                break
        ...
        if not torrent_found:
            console.print(f"[bold red]Warning: Torrent with hash {torrent.infohash} not found in qBittorrent after adding.")
        else:
