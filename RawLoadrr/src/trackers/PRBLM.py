...
class PRBLM(TorFallbackMixin):
    def __init__(self, config):
        self.config = config
        self.tracker = 'PRBLM'
        self.source_flag = 'PARABELLUM'
        self.upload_url = 'https://parabellumhd.cx/api/torrents/upload'
        self.search_url = 'https://parabellumhd.cx/api/torrents/filter'
        self.banned_groups = [""]
    ...
    async def upload(self, meta):
        ...
        data = {
            'name' : manual_name or self.prblm_name(meta),
            'description' : desc,
            'mediainfo' : mi_dump,
            'bdinfo' : bd_dump, 
            'category_id' : cat_id,
    ...
    def prblm_name(self, meta):
        ...
        if meta.get('original_language', '') in ('es', 'spa') and og_title:
            prblm_name = built_name.replace(title, og_title).strip()
        else: 
            prblm_name = built_name
            
        # Validate length
        while len(prblm_name) > 255:
            original_len = len(prblm_name)
            if aka:
                prblm_name = prblm_name.replace(aka, '')
            if len(prblm_name) <= 255:
                break
            
            resolution = meta.get('resolution', '')
            if resolution:
                prblm_name = prblm_name.replace(resolution, '')
            
            if len(prblm_name) <= 255:
                break
            type = meta.get('type', '')
            if type:
                prblm_name = prblm_name.replace(type, '')
            if len(prblm_name) <= 255:
                break
            
            if len(prblm_name) == original_len:
                # Break if no change in length to prevent infinite loop
                break
        return prblm_name[:255]
