...
echo "--- 🛠️  Iniciando forja de la imagen v3.0.0 (Limpia y sin cache) ---"

# 1. Construir ignorando la cache para asegurar frescura total
docker build --no-cache -t rawsmoke/singularity-suite:v3.0.0 .
...
# 3. Subir al Hub
echo "--- ☁️  Subiendo el Tanque (v2.4.0 & latest) a Docker Hub... ---"
docker push rawsmoke/singularity-suite:v3.0.0
