...
echo "--- 🛠️  Iniciando forja de la imagen v3.1.1 (Limpia y sin cache) ---"

# 1. Construir ignorando la cache para asegurar frescura total
docker build --no-cache -t rawsmoke/singularity-suite:v3.1.1 .

# 2. Taggear como latest para despliegues estándar
docker tag rawsmoke/singularity-suite:v3.1.1 rawsmoke/singularity-suite:latest

# 3. Subir al Hub
echo "--- ☁️  Subiendo el Tanque (v3.1.1 & latest) a Docker Hub... ---"
docker push rawsmoke/singularity-suite:v3.1.1
