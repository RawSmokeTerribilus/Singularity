...
# openbox = WM mínimo en el display virtual: sin WM, Chrome --kiosk degrada a
# ventana normal CON toolbar (la barra de URL se graba) y F11 no tiene vuelta.
RUN apt-get update && apt-get install -y --no-install-recommends gnupg ca-certificates && \
    ...
    xvfb x11-utils x11vnc xauth openbox \
